...
function OptionButton({ currentValue, set, children, lang }: { currentValue: boolean; set: () => void; children: ReactNode | ReactNode[]; lang: Language }) {
  return (
    <li>
      <button onClick={set}>
        {children}
...
export function TopBar({ options, setOptions, setAppState }: { options: Options; setOptions: (v: Partial<Options>) => void; setAppState: (v: Partial<AppState>) => void }) {
  return (
        ...
        <li className="menu-item">
          ...
          <ul className="dropdown">
            ...
            </li>
            {
              <li className="has-submenu">
                ...
                <ul className="submenu">
                  ...
                  <OptionButton currentValue={options.language === 'nl'} set={() => setOptions({ language: 'nl' })} lang={options.language}>
                    Nederlands
                  </OptionButton>
                </ul>
              </li>
            ...
            <li className="has-submenu">
              ...
              <ul className="submenu">
                <OptionButton currentValue={options.resultAccuracy === 2} set={() => setOptions({ resultAccuracy: 2 })} lang={options.language}>
                  2 {translate('optionsPrecisionNumber', options.language)}
                </OptionButton>
                <OptionButton currentValue={options.resultAccuracy === 3} set={() => setOptions({ resultAccuracy: 3 })} lang={options.language}>
                  3 {translate('optionsPrecisionNumber', options.language)}
                </OptionButton>
                <OptionButton currentValue={options.resultAccuracy === 8} set={() => setOptions({ resultAccuracy: 8 })} lang={options.language}>
                  8 {translate('optionsPrecisionNumber', options.language)}
                </OptionButton>
                <OptionButton currentValue={options.resultAccuracy === 15} set={() => setOptions({ resultAccuracy: 15 })} lang={options.language}>
                  15 {translate('optionsPrecisionNumber', options.language)}
                </OptionButton>
                <OptionButton currentValue={options.resultAccuracy === 50} set={() => setOptions({ resultAccuracy: 50 })} lang={options.language}>
            ...
            <li className="has-submenu">
              ...
              <ul className="submenu">
                <OptionButton currentValue={options.preserveSessions === true} set={() => setOptions({ preserveSessions: true })} lang={options.language}>
                  {translate('optionsYes', options.language)}
                </OptionButton>
                <OptionButton currentValue={options.preserveSessions === false} set={() => setOptions({ preserveSessions: false })} lang={options.language}>
            ...
            {import.meta.env.VITE_ABITTI_BUILD !== 'true' && !window.matchMedia('(display-mode: standalone)').matches && (
              <li className="has-submenu fullscreen-option">
                ...
                <ul className="submenu">
                  <OptionButton currentValue={options.fullScreen === true} set={() => setOptions({ fullScreen: true })} lang={options.language}>
                    {translate('optionsYes', options.language)}
                  </OptionButton>
                  <OptionButton currentValue={options.fullScreen === false} set={() => setOptions({ fullScreen: false })} lang={options.language}>
                    {translate('optionsNo', options.language)}
                  </OptionButton>
                </ul>
              </li>
          ...
          <ul className="dropdown">
            <li>
              <a href={translate('feedbackLink', options.language)}>
                <Smile size={'1.25rem'} /> {translate('helpSendFeedback', options.language)}
              </a>
            </li>
            <li>
              <button onClick={() => setAppState({ pageOpen: 'about' })}>
                <Info size={'1.25rem'} /> {translate('helpInfo', options.language)}
              </button>
            </li>
            <li>
              <button onClick={() => setAppState({ pageOpen: 'copyright' })}>
                <Copyright size={'1.25rem'} /> {translate('helpCopyrights', options.language)}
              </button>
            </li>
